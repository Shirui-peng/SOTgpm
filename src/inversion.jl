...
```
"""
function collectpairs(eqname, tsname, tstations, tintervals, tavgwidth, treffreq, tinvfreq, tmincc,
                      pstations, pintervals, pfreqbands; soundspeed=1.5e3, tmaxΔτ=Inf, excludetimes=[], r2=true, l2=false,
  ...
  for i = 1 : size(pstations, 1)
    ...
    push!(ppairs, DataFrame(CSV.File(filename, select=1:10)))
  ...
  CSV.write(@sprintf("results/pairs/japan_%s_ppairs_full.csv",tstations[1]), ppairs)
  ...
  if l2
    tpairs = DataFrame(station=String[], event1=DateTime[], event2=DateTime[],
                       ...
                       Δτr=Array{Float64,1}[], Δτr2=Array{Float64,1}[], Δτl2=Array{Float64,1}[], 
                       ccl=Array{Float64,1}[], ccc=Array{Float64,1}[], 
                       ccr=Array{Float64,1}[])
  elseif r2
    tpairs = DataFrame(station=String[], event1=DateTime[], event2=DateTime[],
                       Δτl=Array{Float64,1}[], Δτc=Array{Float64,1}[],
                       Δτr=Array{Float64,1}[], Δτr2=Array{Float64,1}[], 
                       ccl=Array{Float64,1}[], ccc=Array{Float64,1}[], 
                       ccr=Array{Float64,1}[])#, ccr2=Array{Float64,1}[])
  else
    tpairs = DataFrame(station=String[], event1=DateTime[], event2=DateTime[],
                       Δτl=Array{Float64,1}[], Δτc=Array{Float64,1}[],
                       Δτr=Array{Float64,1}[], 
  ...
  @showprogress for i = 1 : length(tstations), j = 1 : size(ppairs, 1)
    ...
    if isfile(tdelayfile) && filesize(tdelayfile) > 0
      ...
      if all(ccc .> tmincc) && all(abs.(Δτc) .< tmaxΔτ) #if all(abs.(Δτc) .< tmaxΔτ) && all(ccc .> tmincc) && all(ccr .> 0.) && all(ccl .> 0.)
        
        # record measurements
        try
            if l2
              push!(tpairs, [tstations[i], ppairs[j,:event1], ppairs[j,:event2],
                             read(fid, "Δτl")[idx], Δτc, read(fid, "Δτr")[idx], read(fid, "Δτr2")[idx], read(fid, "Δτl2")[idx],
                             read(fid, "ccl")[idx], ccc, read(fid, "ccr")[idx]])
            elseif r2
              push!(tpairs, [tstations[i], ppairs[j,:event1], ppairs[j,:event2],
                             read(fid, "Δτl")[idx], Δτc, read(fid, "Δτr")[idx], read(fid, "Δτr2")[idx], 
                             read(fid, "ccl")[idx], ccc, read(fid, "ccr")[idx]])
            else
              push!(tpairs, [tstations[i], ppairs[j,:event1], ppairs[j,:event2],
  ...
  ppairs = DataFrame(station=String[],
                     event1=DateTime[], latitude1=Float64[], longitude1=Float64[], depth1=Float64[], magnitude1=Float64[],
...
```
"""
function invert0(tpairs, ppairs, λ, σc, σn, σp, U, Λ, Δz, h; σtrend=0, σannual=0, σsemiannual=0)
  ...
  R = [T*C*T' zeros(l*m, m); zeros(m, (l+1)*m)] + σp^2*kron(sparse(ones(l+1, l+1)), I(m))
...
function invert(tpairs, ppairs, tstation, pstations, evtpos, σc,σp,σs,σn,σnp,σh,lags,ctau; σtrend=0, σannual=0, σsemiannual=0)
...
function invertf1(tpairs, ppairs, tstation, pstations, evtpos, λt, λθ, στ, σx, σh, σn, σnp, σp; σtrend=0, σannual=0, σsemiannual=0, σθtrend=0)
...
*T*-wave delays `Δτ`.
"""
function correctcycleskipping0(tpairs, ppairs, E, Rxx, Rnn, P, m)
...
function correctcycleskipping(eqname, tstations, tpairs, ppairs, E, Rxx, Rnn, invR, P, m)
...
function correctcycleskippingf1r2(eqname, tstations, tpairs, ppairs, E, Rxx, Rnn, P, m; l2=false)
...
function regulargrid0(td, ti, a, R, λ, h, U, Λ, σc, P)
  m = length(td)
  l = length(Λ)
  ...
  ci = Ti*F*iR*a#, (length(ti), l)) + a[(l+1)*m+1:(l+1)*m+l]'.*(tid.-tm) + a[(l+1)*m+l+1:(l+1)*m+2l]'.*cos.(ω*tid) + a[(l+1)*m+2l+1:(l+1)*m+3l]'.*sin.(ω*tid) + a[(l+1)*m+3l+1:(l+1)*m+4l]'.*cos.(2ω*tid) + a[(l+1)*m+4l+1:(l+1)*m+5l]'.*sin.(2ω*tid)
...
""" interpolation onto regular grid of averages """
function regulargrid(td, θd, ti, θi, a, R, iRyy, E, λt, λθ, στ)
  @printf("\n")
  ...
  #F = F*invR + [zeros(l*length(tidv),(l+1)*m) (tidv.-tm) cos.(ω*tidv) sin.(ω*tidv) cos.(2ω*tidv) sin.(2ω*tidv)]
  @printf("calculate regular anomalies...\n")
  #τi = reshape(F*p, (length(tidv), l)) + a[(l+1)*m+1]*(tidv.-tm) + a[(l+1)*m+l+1]*cos.(ω*tidv) + a[(l+1)*m+2l+1]*sin.(ω*tidv) + a[(l+1)*m+3l+1]*cos.(2ω*tidv) + a[(l+1)*m+4l+1]*sin.(2ω*tidv)
...
""" interpolation onto regular grid of averages """
function regulargrida(td, ti, a, y, R, E, N, invR, P, σc, ctau, lags)
  @printf("\n")
  @printf("interpolation onto regular grid\n")
  m = length(td)
  mi = length(ti)
  ...
  #Di = [I(l*length(ti)) I(l*length(ti))]
  τi = F*E'*inv(Matrix(N+E*R*E'))*y # reshape(F*p, (length(ti), l)) + a[(l+1)*m+1:(l+1)*m+l]'.*(tid.-tm) 
  F = F*invR#; zeros(l*length(ti),(l+1)*m) kron(I(l), tid.-tm)]
  τi2 = F*a 
  Pi = F*P*F'
  ...
  return reshape(A*τi, (mi, l)),A*Pi*A',C,zix,zix2#A*(C-ΔC)*A',zix,zix2
end