...
using Random, Distributions, SparseArrays, PyPlot, Interpolations
...
te = DateTime(2008, 1, 1):Day(10):DateTime(2021, 7, 31)
m = length(te)
lone = ones(m)*h11a0[1]
late = ones(m)*h11a0[2]#h5read("data/japan_H11N3_x2y_16ex.h5", "lat")
azme = zeros(m)#h5read("data/japan_H11N3_x2y_16ex.h5", "θ")
...
tt = DateTime(2000, 1, 1) .+ Millisecond.(h5read("results/anomalies/japan_H11N3_x2y_16ex.h5", "t"))
...
t0,t1,x0,y0,z0 = DateTime(2007,12,16),DateTime(2021,8,1),141,36,-1.9e3
...
#Pa,xa,Pt,xt,Pat,xat,C0 = SOT.invertargottc(te,azme,x,3.7σtrend, σannual, σsemiannual,tpairs,ppairs,tstation,evtpos,pstations,nearfloats;itp_c,itp_ca,itp_tc,itp_tca,mode)
...
h5open("results/argo/point_H11_0d.h5", "w") do file
  write(file, "xa", xa)
  write(file, "xt", xt)
  write(file, "xat", xat)
  write(file, "Pa", Pa)
