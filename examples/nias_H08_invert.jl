...
fl = @sprintf("results/%s/covtau0_%s.h5",model,Kgrid)
lags = h5read(fl, "lags")
ctau = h5read(fl, "covs")
...
t, E, R, N, P, D, invR = SOT.invert(tpairs, ppairs, tstalocs, pstalocs, evtpos, σc,σp,σs,σn,σnp,σh,lags,ctau; σtrend)

# make cycle-skipping correction
tpairs.Δτ,tpairs.Δτp,zint,zinp = SOT.correctcycleskipping(source, tstations, tpairs, ppairs, E, R, N, invR, P, m)
...
τi,Pi,ΔCi,zix,zix2 = SOT.regulargrida(td, ti, a, R, invR, P, σc, ctau, lags)
