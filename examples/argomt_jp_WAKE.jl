...
t0,t1,y0,z0 = DateTime(1997,6,5),DateTime(2021,8,1),30,-1.9e3
...
for (j,y) in enumerate(yrg)
  if 15<y<45
    ...
    Pa,xa,Pt,xt,Pat,xat = SOT.invertargotmt(gxy,x,3.7σtrend,σannual,σsemiannual,tm,floatsj,tpairs,ppairs,tstation,pstations,evtpos,itp_c,itp_tc,itp_ca,itp_tca)
...
# save to file
h5open("results/argo/kuroshio_mt_WAKE.h5", "w") do file
  write(file, "x", xrg)
  write(file, "y", xrg)
  write(file, "gmn", gmn)
  write(file, "gtd", gtd)
